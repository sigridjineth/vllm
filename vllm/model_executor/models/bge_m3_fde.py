...
class BgeM3FDE(RobertaEmbeddingModel):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.pooler = FDEPooler(d=fde_input_dim, config=self.fde_config)
    ...
    def _load_fde_params(self):
        ...
        try:
            ...
            missing, unexpected = self.pooler.load_state_dict(new_state_dict,
                                                              strict=False)
            if missing:
            ...
            # Verify shape consistency for G
            if "params.G" in new_state_dict:
                loaded_shape = new_state_dict["params.G"].shape
                expected_shape = self.pooler.params.G.shape
